...
async def send_medias(illust: str, page, session: sqlalchemy.orm.Session, context: ContextTypes.DEFAULT_TYPE, send_preview: Union[List, bytes], send_file: List, introduce, is_ugoira, have_sent: List, spoiler=False):
    ...
    for channel in channels:
        if not is_ugoira:
            preview_message = await context.bot.sendMediaGroup(chat_id=channel, media=send_preview, parse_mode='HTML'
                                                               , caption=introduce
...
async def send_illust(illust: db.Illust, session: sqlalchemy.orm.Session, context: ContextTypes.DEFAULT_TYPE, have_sent: List):
...
async def delete_backup(illust: Union[str, db.Illust], session: sqlalchemy.orm.Session, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in backup_messages:
        await context.bot.deleteMessage(chat_id=i.channel, message_id=i.message_id)
